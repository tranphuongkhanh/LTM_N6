...
await playhtml.init({ room: `ott-${ROOM_CODE}` });
...
function performMove(from, to, moveType) {
  ...
  channel.setData(draft => {
    const rawPiece = draft.board[from.r][from.c];
    if (!rawPiece) return;
    const piece = { type: rawPiece.type, player: rawPiece.player };
    const rawCaptured = draft.board[to.r][to.c];
    const capturedPiece = rawCaptured ? { type: rawCaptured.type, player: rawCaptured.player } : null;
    ...
    draft.board[to.r].splice(to.c, 1, piece);
    draft.board[from.r].splice(from.c, 1, null);
    ...
    if (moveType === 'capture' && capturedPiece) entry += ` (ăn ${TYPE_LABEL[capturedPiece.type]})`;
...
function undo() {
  ...
  channel.setData(draft => {
    ...
    for (let r = 0; r < SIZE; r++) {
      draft.board[r].splice(0, SIZE, ...clean.board[r]);
    }
    draft.counts = clean.counts;
    draft.currentPlayer = clean.currentPlayer;
    draft.moveLog.splice(0, draft.moveLog.length, ...clean.moveLog);
    draft.gameOver = clean.gameOver;
    draft.winner = clean.winner;
...
function newGame() {
  channel.setData(draft => {
    ...
    for (let r = 0; r < SIZE; r++) {
      draft.board[r].splice(0, SIZE, ...fresh.board[r]);
    }
    draft.counts = fresh.counts;
    draft.currentPlayer = fresh.currentPlayer;
    draft.moveLog.splice(0, draft.moveLog.length);
